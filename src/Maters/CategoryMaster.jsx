...
const tokens = (dark) => ({
  ...
  pageBg:     dark ? "rgba(255,255,255,0.06)"  : "rgba(22,199,132,0.08)",
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            ...
            <Box>
              ...
              <Typography variant="body2" sx={{ color: T.muted, fontSize: "0.78rem" }}>
                ...
                {totalRows > 0 && (
                  <span style={{ marginLeft: 8, color: "#16c784", fontWeight: 600 }}>
                    · {totalRows} total
                  </span>
                )}
              </Typography>
            ...
            <table style={{ width: "100%", fontSize: "0.85rem", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8} style={{ textAlign: "center", padding: 48, color: T.muted }}>
                  ...
                  categories.map((row, i) => (
                    ...
                    >
                      ...
                      <td style={tdRow}>
                        <Chip label={row.created_by || "System"} size="small" sx={{ background: "#dcfce7", color: "#16a34a", fontWeight: 600, fontSize: "0.7rem" }} />
                      </td>
                      <td style={{ ...tdRow, color: T.muted, whiteSpace: "nowrap" }}>{fmtDate(row.created_date)}</td>
                      <td style={tdRow}>
                        <Chip label={row.last_modified_by || "—"} size="small" sx={{ background: dark ? "#1a2e1a" : "#f0fdf4", color: "#15803d", fontSize: "0.7rem" }} />
                      </td>
                      <td style={{ ...tdRow, color: T.muted, whiteSpace: "nowrap" }}>{fmtDate(row.last_modified_date)}</td>
                      <td style={tdRow}>
                        <Box sx={{ display: "flex", gap: 0.5 }}>
                          <Tooltip title="Edit">
                            <IconButton size="small" onClick={() => openEdit(row)} sx={{ color: "#16c784", "&:hover": { background: "#dcfce7" } }}>
                              <EditIcon sx={{ fontSize: 17 }} />
                            </IconButton>
                          </Tooltip>
                          <Tooltip title="Delete">
                            <IconButton size="small" onClick={() => setDeleteId(row.id)} sx={{ color: "#ef4444", "&:hover": { background: "#fee2e2" } }}>
              ...
              <Box sx={{ position: "fixed", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", p: { xs: 1.5, sm: 2 }, backdropFilter: "blur(6px)", backgroundColor: "rgba(0,0,0,0.45)" }}>
                ...
                >
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                    <TextField
                      label="Category Name" {...register("category_name")}
                      error={!!errors.category_name} helperText={errors.category_name?.message}
                      fullWidth autoFocus size="small" sx={T.inputSx}
                      InputProps={{ style: { color: T.text } }} InputLabelProps={{ style: { color: T.muted } }}
                    />
                    <TextField
                      label="Category Description" {...register("category_description")}
                      error={!!errors.category_description} helperText={errors.category_description?.message}
                      fullWidth multiline rows={3} size="small" sx={T.inputSx}
                      InputProps={{ style: { color: T.text } }} InputLabelProps={{ style: { color: T.muted } }}
                    />
